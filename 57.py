...
contents = Path('./data/an_158-0.txt').read_text()
sentences = rt.split_into_sentences(contents)
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
	...
	for ii in range(3000):
		for j in range(1000):
			if j % 100 == 0:
				print("Current state : ii = %d, j=%d" % (ii, j))
				print("Current sentences :", sentences[j])
				
			if len(sentences[j].split(' ')) > 25 or len(sentences[j].split(' ')) < 5:
				continue

			e_text, em_text = encoding_text([sentences[j]], idx2word, word2idx, idx2w_embedding)
			...
			res_outputs, _res_states = sess.run([outputs, _states], feed_dict=feed_Encoder)
			
			res_states = list()
			for _h in _res_states.h:
				res_states.append(list(_h))
			...
			if j % 100 == 0 or j % 30 == 0 or j % 60 == 0:
				print("loss : ", l)
		...
		print(str(ii) + " cycle is done")
		ckpt_name = "./02_" + str(ii) + ".ckpt"
